...
public:
    ...
    void setPupilHighlightShow(const bool);
...
Pupil::Pupil() : show_pupil_highlight_(true)
{
...
void Pupil::setPupilHighlightShow(const bool show)
{
    show_pupil_highlight_ = show;
}

void Pupil::setSquircleRadius(const sf::Vector2f squircle_radius)
{
    squircle_shape_.setRadius(sf::Vector2f(PUPIL_SIZE / 2.0f * squircle_radius.x, PUPIL_SIZE / 2.0f * squircle_radius.y));
}

void Pupil::show(const bool show)
{
    show_ = show;
}

void Pupil::setTransformation(const sf::Transform transform)
{
    target_transformation_ = transform;
}

void Pupil::setColour(const sf::Color colour)
{
    Entity::setColour(colour);
    squircle_shape_.setFillColor(colour);
}

void Pupil::draw(sf::RenderWindow &renderWindow, const float frame_delta_time)
{
